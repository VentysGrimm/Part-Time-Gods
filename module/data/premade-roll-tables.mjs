...
function archetypeOptionTables() {
  return PTG_PREMADE_CHOICES
    ...
    .flatMap(choice => {
      const name = archetypeTableName(choice.name);
      const options = choice.system ?? {};

      return [
        table(`Random Archetype Attachment - ${name}`, "1d10", 283, twoOptionRows(
          options.attachmentOptions ?? [],
          archetypeAttachmentResult
        )),
        table(`Random Archetype Blessing - ${name}`, "1d10", 283, threeOptionRows(
          options.blessingOptions ?? [],
          option => option.name
        )),
        table(`Random Archetype Curse - ${name}`, "1d10", 283, twoOptionRows(
...
function archetypeTableName(name) {
  return String(name ?? "").replace(/^The\s+/i, "");
}

function twoOptionRows(options, labeler) {
  if (options.length < 2) return [[1, reroll], [[2, 10], gmChoice]];
  return [
    [[1, 5], labeler(options[0])],
    [[6, 10], labeler(options[1])]
  ];
}

function threeOptionRows(options, labeler) {
  if (options.length < 3) return [[1, reroll], [[2, 10], gmChoice]];
  return [
    [1, reroll],
    [[2, 4], labeler(options[0])],
    [[5, 7], labeler(options[1])],
    [[8, 10], labeler(options[2])]
  ];
}

function archetypeAttachmentResult(option) {
  const level = Number(option?.level ?? 1);
  return `+${level} ${attachmentLabel(option?.kind, option?.name)}`;
}

function attachmentLabel(kind, name) {
  if (["individual", "group", "landmark"].includes(kind)) {
    return `${labelize(kind)} Bond`;
  }

  return String(name ?? labelize(kind ?? "attachment")).replace(/\s+Entitlement$/i, "");
}
