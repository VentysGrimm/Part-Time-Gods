  ...
  const content = `
    <div class="ptg-combat-dialog">
      ...
      <div class="ptg-item-fields two">
        <label class="ptg-checkbox">
          <span>Roll attacker statblock Attack pool</span>
          <input type="checkbox" name="rollAttack">
        </label>
        <label class="ptg-checkbox">
          <span>Roll defender statblock Defense pool</span>
          <input type="checkbox" name="rollDefense">
        </label>
      </div>
      <div class="ptg-item-fields two">
        <div class="form-group">
    ...
    ok: {
      ...
      callback: (event, button) => {
        ...
        return {
          ...
          rollAttack: form.elements.rollAttack?.checked ?? false,
          rollDefense: form.elements.rollDefense?.checked ?? false,
...
async function resolveBattleOutcome(attacker, defender, selection) {
  ...
  const attackRoll = selection.rollAttack ? await rollStatblockSuccesses(attacker, "attack") : null;
  const defenseRoll = selection.rollDefense ? await rollStatblockSuccesses(defender, "defense") : null;
  const attackSuccesses = Math.max(0, Number(attackRoll?.successes ?? selection.attackSuccesses ?? 0));
  const defenseModifier = conditionCombatModifier(defender, "defense");
  const defenseBaseSuccesses = Math.max(0, Number(defenseRoll?.successes ?? selection.defenseSuccesses ?? 0));
  const defenseSuccesses = Math.max(0, defenseBaseSuccesses + defenseModifier);
  ...
  if (selection.rollAttack) {
    results.push(attackRoll
      ? `${attacker.name}: Attack statblock pool ${attackRoll.pool}d10 rolled ${attackRoll.successes} success${attackRoll.successes === 1 ? "" : "es"}.`
      : `${attacker.name}: no Attack statblock pool was available; manual attack successes were used.`);
  }
  if (selection.rollDefense) {
    results.push(defenseRoll
      ? `${defender.name}: Defense statblock pool ${defenseRoll.pool}d10 rolled ${defenseRoll.successes} success${defenseRoll.successes === 1 ? "" : "es"}.`
      : `${defender.name}: no Defense statblock pool was available; manual defense successes were used.`);
  }
...
async function rollStatblockSuccesses(actor, stat) {
  ...
  return { roll, pool, successes };
}

function statblockPool(actor, stat) {
  const system = actor?.system ?? {};
  const value = Number(system[stat] ?? 0);
...
function roundSequenceHTML() {
  return `
    <ol>
      ...
      <li>Resolve Battle of Fists or Battle of Wits with attack and defense successes, rolling NPC statblock pools directly when useful.</li>
...
function battleOutcomeHTML(results, selection, attacker, defender) {
  return `
    ...
    <div><strong>Attack / Defense:</strong> ${selection.rollAttack ? "statblock roll" : Number(selection.attackSuccesses ?? 0)} / ${selection.rollDefense ? "statblock roll" : Number(selection.defenseSuccesses ?? 0)}</div>
