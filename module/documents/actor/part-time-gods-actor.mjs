...
export class PartTimeGodsActor extends Actor {
  ...
  async applyChoice(item, options = {}) {
    ...
    if (options.confirm !== false) {
      const confirmed = await confirmChoiceApplication(this, item, {
        key,
        updates,
        embedded,
        careerSelection,
    ...
    if (item.parent?.uuid === this.uuid) {
      await item.update(canonicalFlagUpdates(item, "applied-choice", { appliedChoiceKey: key }));
    }
  ...
  async useOwnedItem(item) {
    ...
    if (item.type === "worshipper") return this.requestWorshipperPrayer(item);
    if (item.type === "vassal") return this.requestVassalTask(item);

    const rollSelection = item.system.automation?.enabled && item.system.automation?.roll
      ? await this.#selectItemRollAutomation(item)
      : null;
    if (rollSelection === false) return false;
    if (rollSelection?.pantheonDice > 0) {
      const pantheon = actorResource(this, "pantheon");
      if (!pantheon || pantheon.value < rollSelection.pantheonDice) {
        ui.notifications.warn("Not enough Pantheon Dice.");
        return false;
      }
    }
    ...
    if (item.type === "weapon") {
      ...
      if (item.system.automation?.enabled) {
        const automationResults = await this.#applyItemAutomation(item, rollSelection);
    ...
    const automationResults = item.system.automation?.enabled
      ? await this.#applyItemAutomation(item, rollSelection)
      : [];
    const results = [
      ...costResults,
      ...automationResults
  ...
  async requestAttachmentAction(item, action = "favor") {
    ...
    if (item.type === "vassal" && action === "favor") return this.requestVassalTask(item);
  ...
  async requestVassalTask(item, preset = {}) {
    if (!item || item.type !== "vassal") return false;

    const selection = await selectVassalTask(item, preset);
    if (!selection) return false;
    ...
    if (selection.strainDelta) {
      const changed = await this.adjustAttachmentStrain(item, selection.strainDelta, "Vassal Task Risk");
    ...
    let outcome = null;
    if (selection.rollPool > 0) {
      outcome = await PTGDiceEngine.rollPool(selection.rollPool + selection.bonus - selection.penalty, {
        basePool: selection.rollPool,
        bonus: selection.bonus,
        penalty: selection.penalty,
        difficulty: selection.difficulty,
        checkMode: "vassal-task",
        flavor: `${item.name}: ${labelCase(selection.taskType)}`
      });
      results.push(`${item.name}: ${outcome.successes} success${outcome.successes === 1 ? "" : "es"} vs Difficulty ${outcome.difficulty}.`);
    }
    ...
    const entry = {
      ...
      rollStat: selection.rollStat,
      rollPool: selection.rollPool,
      difficulty: selection.difficulty,
      successes: outcome?.successes ?? null,
      passed: outcome?.passed ?? null,
      createdAt: new Date().toISOString()
    };

    await item.update({
      "system.currentTask": selection.task,
    ...
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ actor: this }),
      content: vassalTaskCard(this, item, selection, results, outcome)
    });
  ...
  async #applyItemAutomation(item, rollSelection = null) {
    const automation = item.system.automation ?? {};
    const results = [];

    if (rollSelection) {
      const result = await this.#applyRollAutomation(item, rollSelection);
  ...
  async #selectItemRollAutomation(item) {
    const roll = item.system.automation?.roll ?? {};
    const type = roll.type ?? item.system.automation?.action ?? "";
    const skillEntries = Object.entries(CONFIG.PTG.skills ?? {});
    ...
    const defaultSkill = roll.primary ?? roll.skill ?? "discipline";
    const defaultSecondary = roll.secondary ?? "intuition";
    const defaultManifestation = roll.manifestation ?? item.system.manifestation ?? defaultSecondary;
    const difficulty = Number(roll.difficulty ?? item.system.difficulty ?? 1);
    const isManifestation = type === "manifestation" || type === "manifestation-roll";
    const isRitual = type === "ritual" || type === "ritual-roll";
    const isOtherworld = type === "otherworld-stage" || type === "otherworld-stage-roll";
    const title = isRitual
      ? `${item.name}: Ritual`
      : isOtherworld
        ? `${item.name}: Otherworld Travel`
        : `${item.name}: Power Roll`;

    const content = `
      <div class="ptg-roll-dialog">
        <div class="ptg-territory-summary">
          <strong>${escapeHTML(title)}</strong>
          <span>${escapeHTML(item.system.rules?.summary || item.system.usage?.trigger || "Resolve this item workflow.")}</span>
        </div>
        ${isManifestation || isRitual ? `
          <div class="form-group">
            <label>Manifestation</label>
            <select name="manifestation">${manifestationEntries.map(([key, label]) => `<option value="${escapeHTML(key)}" ${key === defaultManifestation ? "selected" : ""}>${escapeHTML(label)} (${Number(this.system.manifestations?.[key] ?? 0)})</option>`).join("")}</select>
          </div>
          <div class="form-group">
            <label>${isRitual ? "Ritual Skill" : "Skill"}</label>
            <select name="primary">${skillEntries.map(([key, label]) => `<option value="${escapeHTML(key)}" ${key === defaultSkill ? "selected" : ""}>${escapeHTML(label)} (${Number(this.system.skills?.[key] ?? 0)})</option>`).join("")}</select>
          </div>
        ` : `
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select name="secondary">${skillEntries.map(([key, label]) => `<option value="${escapeHTML(key)}" ${key === defaultSecondary ? "selected" : ""}>${escapeHTML(label)} (${Number(this.system.skills?.[key] ?? 0)})</option>`).join("")}</select>
          </div>
        `}
        <div class="form-group">
          <label>Difficulty</label>
          <select name="difficulty">${difficulties.map(([key, value]) => `<option value="${Number(value)}" ${Number(value) === difficulty ? "selected" : ""}>${escapeHTML(`${labelCase(key)} (${value})`)}</option>`).join("")}<option value="custom">Custom</option></select>
        </div>
        <div class="form-group">
          <label>Custom Difficulty</label>
          <input type="number" name="customDifficulty" value="${difficulty}" min="0">
        </div>
        <section class="ptg-item-fields three">
          <label>
            <span>Bonus</span>
          ...
          <label>
            <span>Pantheon Dice</span>
            <input type="number" name="pantheonDice" value="0" min="0">
          </label>
        </section>
        <label>
          <span>${isOtherworld ? "Route / Stage Objective" : isRitual ? "Ritual Objective" : "Power Objective"}</span>
          <textarea name="objective" rows="3" placeholder="Target, destination, participants, desired effect, or table ruling"></textarea>
        </label>
        <label>
          <span>Boost Choice</span>
        ...
        <label>
          <span>Notes</span>
          <textarea name="notes" rows="3" placeholder="Cost progress, travel consequence, resistance, interruption, or GM ruling"></textarea>
        </label>
      </div>
    `;
    ...
    const selection = await DialogV2.prompt({
      ...
      ok: {
        ...
        callback: (event, button) => {
          ...
          return {
            type: isRitual ? "ritual" : isOtherworld ? "otherworld-stage" : "manifestation",
            manifestation: form.elements.manifestation?.value ?? defaultManifestation,
            primary: form.elements.primary?.value ?? defaultSkill,
            secondary: form.elements.secondary?.value ?? defaultSecondary,
            ...
            pantheonDice: Math.max(0, Number(form.elements.pantheonDice?.value ?? 0)),
  ...
  async #applyRollAutomation(item, selection) {
    if (!selection) return "";

    if (selection.pantheonDice > 0) {
      const spent = await this.spendResource("pantheon", selection.pantheonDice);
      if (!spent) return "Roll canceled: Pantheon Dice could not be spent.";
    }

    const common = {
      difficulty: Math.max(0, Number(selection.difficulty ?? 0)),
      bonus: Number(selection.bonus ?? 0),
      penalty: Number(selection.penalty ?? 0),
      boostChoice: selection.boostChoice,
      applyConditionModifiers: selection.applyConditionModifiers,
      modifierDetails: {
        "Pantheon Dice": Number(selection.pantheonDice ?? 0)
      },
      checkMode: selection.type,
      flavor: `${this.name}: ${item.name}`
    };

    let outcome = null;
    let rollLabel = "";
    if (selection.type === "manifestation") {
      outcome = await this.rollManifestation(selection.manifestation, selection.primary, {
        ...common,
        checkMode: "manifestation"
      });
      rollLabel = `${manifestationLabel(selection.manifestation)} + ${skillLabel(selection.primary)}`;
    } else if (selection.type === "ritual") {
      outcome = await this.rollManifestation(selection.manifestation, selection.primary, {
        ...common,
        checkMode: "ritual"
      });
      rollLabel = `${skillLabel(selection.primary)} + ${manifestationLabel(selection.manifestation)}`;
    } else {
      outcome = await this.rollSkillCombo(selection.primary, selection.secondary, {
        ...common,
        checkMode: "otherworld-stage"
      });
      rollLabel = `${skillLabel(selection.primary)} + ${skillLabel(selection.secondary)}`;
    }

    const pieces = [
      `${rollLabel}: ${outcome.successes} success${outcome.successes === 1 ? "" : "es"} vs Difficulty ${outcome.difficulty}.`,
      outcome.passed ? "Workflow roll succeeded." : "Workflow roll failed; apply the listed ritual, travel, or power consequence.",
...
async function selectVassalTask(item, preset = {}) {
  const templateSystem = item.system.actorTemplate?.system ?? {};
  const statOptions = [
    ["", "No statblock roll"],
    ["attack", `Attack (${Number(templateSystem.attack ?? 0)})`],
    ["defense", `Defense (${Number(templateSystem.defense ?? 0)})`],
    ["initiative", `Initiative (${Number(templateSystem.initiative ?? 0)})`],
    ["damage", `Damage (${Number(templateSystem.damage ?? 0)})`],
    ["threat", `Threat (${Number(templateSystem.threat ?? item.system.level ?? 1)})`],
    ["custom", "Custom pool"]
  ];
  const content = `
    <div class="ptg-advancement-dialog">
      <div class="ptg-territory-summary">
        <strong>${escapeHTML(item.name)}</strong>
        <span>${escapeHTML(templateSystem.rank || item.system.concept || "Vassal task")}${templateSystem.threshold ? `; Threshold ${Number(templateSystem.threshold)}` : ""}</span>
      </div>
      <div class="form-group">
        <label>Task Type</label>
        <select name="taskType">
          <option value="battle-support">Battle Support</option>
          <option value="guard">Guard / Protect</option>
          <option value="scout">Scout / Track</option>
          <option value="travel">Travel / Escort</option>
          <option value="negotiate">Negotiate / Intimidate</option>
      ...
      <div class="form-group">
        <label>Task</label>
        <textarea name="task" rows="3" placeholder="What the god directs this Vassal to do">${escapeHTML(preset.task ?? "")}</textarea>
      </div>
      <div class="form-group">
        <label>Target / Scene</label>
        <input name="target" type="text" value="${escapeHTML(preset.target ?? "")}" placeholder="Enemy, location, route, Bond, or scene">
      </div>
      <section class="ptg-item-fields three">
        <label>
          <span>Risk</span>
        ...
        <label>
          <span>Difficulty</span>
          <input type="number" name="difficulty" value="${Number(preset.difficulty ?? 1)}" min="0">
        </label>
      </section>
      <section class="ptg-item-fields three">
        <label>
          <span>Statblock Pool</span>
          <select name="rollStat">
            ${statOptions.map(([value, label]) => `<option value="${escapeHTML(value)}">${escapeHTML(label)}</option>`).join("")}
          </select>
        </label>
        <label>
          <span>Custom Pool</span>
          <input type="number" name="customPool" value="0" min="0">
        </label>
        <label>
          <span>Bonus / Penalty</span>
          <span class="ptg-inline-number-pair">
            <input type="number" name="bonus" value="0" aria-label="Bonus">
            <input type="number" name="penalty" value="0" aria-label="Penalty">
          </span>
        </label>
      </section>
      <div class="form-group">
        <label>Source Powers</label>
        <div class="ptg-sheet-note">${templateSystem.powers || item.system.relatedBonus || "No statblock powers recorded."}</div>
      </div>
      <div class="form-group">
        <label>Consequence</label>
        <textarea name="consequence" rows="3" placeholder="Risk, cost, loyalty fallout, or opposition response"></textarea>
      </div>
      <div class="form-group">
        <label>Result</label>
        <textarea name="result" rows="3" placeholder="How the task resolves"></textarea>
      </div>
      <div class="form-group">
        <label>GM Notes</label>
        <textarea name="notes" rows="3"></textarea>
  ...
  return DialogV2.prompt({
    window: { title: `${item.name}: Vassal Task`, resizable: true },
    ...
    ok: {
      ...
      callback: (event, button) => {
        const form = button.form;
        const rollStat = form.elements.rollStat?.value ?? "";
        const customPool = Math.max(0, Number(form.elements.customPool?.value ?? 0));
        return {
          taskType: form.elements.taskType?.value ?? "custom",
          task: form.elements.task?.value?.trim() ?? "",
          target: form.elements.target?.value?.trim() ?? "",
          risk: form.elements.risk?.value ?? "none",
          strainDelta: Number(form.elements.strainDelta?.value ?? 0),
          rollStat,
          rollPool: rollStat === "custom" ? customPool : vassalStatPool(item, rollStat),
          difficulty: Math.max(0, Number(form.elements.difficulty?.value ?? 1)),
          bonus: Number(form.elements.bonus?.value ?? 0),
          penalty: Number(form.elements.penalty?.value ?? 0),
          consequence: form.elements.consequence?.value?.trim() ?? "",
...
function vassalTaskCard(actor, item, selection, results, outcome) {
  const templateSystem = item.system.actorTemplate?.system ?? {};
  return `
    <div class="ptg-chat-card">
      <h3>${escapeHTML(item.name)} ${escapeHTML(labelCase(selection.taskType))}</h3>
      <div><strong>Actor:</strong> ${escapeHTML(actor.name)}</div>
      <div><strong>Vassal:</strong> ${escapeHTML(item.system.concept || item.name)} (Level ${Number(item.system.level ?? 1)})</div>
      ${templateSystem.rank ? `<div><strong>Statblock:</strong> ${escapeHTML(templateSystem.rank)}; Threat ${Number(templateSystem.threat ?? item.system.level ?? 1)}; Threshold ${Number(templateSystem.threshold ?? 0)}</div>` : ""}
      ${selection.task ? `<div><strong>Task:</strong> ${escapeHTML(selection.task)}</div>` : ""}
      ${selection.target ? `<div><strong>Target:</strong> ${escapeHTML(selection.target)}</div>` : ""}
      <div><strong>Risk:</strong> ${escapeHTML(labelCase(selection.risk))}</div>
      ${outcome ? `<div><strong>Roll:</strong> ${Number(selection.rollPool)}d10, ${outcome.successes} success${outcome.successes === 1 ? "" : "es"} vs Difficulty ${outcome.difficulty} (${outcome.passed ? "success" : "failure"})</div>` : ""}
      ${selection.consequence ? `<div><strong>Consequence:</strong> ${escapeHTML(selection.consequence)}</div>` : ""}
      ${selection.result ? `<div><strong>Result:</strong> ${escapeHTML(selection.result)}</div>` : ""}
      ${results.length ? `<ul>${results.map(result => `<li>${escapeHTML(result)}</li>`).join("")}</ul>` : ""}
      ${selection.notes ? `<div><strong>GM Notes:</strong> ${escapeHTML(selection.notes)}</div>` : ""}
      ${item.system.powerHooks?.length ? `<div><strong>Power Hooks:</strong> ${escapeHTML(item.system.powerHooks.join(", "))}</div>` : ""}
    </div>
  `;
}

function vassalStatPool(item, stat) {
  if (!stat) return 0;
  const templateSystem = item.system.actorTemplate?.system ?? {};
  if (stat === "threat") return Math.max(0, Number(templateSystem.threat ?? item.system.level ?? 0));
  return Math.max(0, Number(templateSystem[stat] ?? 0));
}
...
async function confirmChoiceApplication(actor, item, context = {}) {
  return DialogV2.confirm({
    window: { title: `${actor.name}: Apply ${item.name}` },
    content: choiceApplicationPreviewHTML(actor, item, context),
    rejectClose: false,
    modal: true
  });
}

function choiceApplicationPreviewHTML(actor, item, context = {}) {
  const choiceLabel = context.careerSelection?.career
    ? `${item.name} - ${context.careerSelection.career.name}`
    : context.domainSelection?.title || item.name;
  const changeRows = choiceUpdateRows(actor, context.updates ?? {});
  const itemRows = (context.embedded ?? []).map(document => `
    <li><strong>${escapeHTML(typeLabel(document.type))}</strong>: ${escapeHTML(document.name)}</li>
  `).join("");
  const branchRows = [
    context.careerSelection?.career ? `Career: ${context.careerSelection.career.name}` : "",
    context.archetypeSelection?.attachment ? `Archetype Attachment: ${attachmentLabel(context.archetypeSelection.attachment)}` : "",
    context.archetypeSelection?.blessing ? `Archetype Blessing: ${context.archetypeSelection.blessing.name}` : "",
    context.archetypeSelection?.curse ? `Archetype Curse: ${context.archetypeSelection.curse.name}` : "",
    context.domainSelection?.blessing ? `Dominion Blessing: ${context.domainSelection.blessing.name}` : "",
    context.domainSelection?.curse ? `Dominion Curse: ${context.domainSelection.curse.name}` : "",
    context.domainSelection?.landmarkName ? `Landmark Bond: ${context.domainSelection.landmarkName}` : "",
    context.theologySelection ? "Undecided Theology custom grants selected." : ""
  ].filter(Boolean).map(row => `<li>${escapeHTML(row)}</li>`).join("");

  return `
    <div class="ptg-choice-preview">
      <p><strong>${escapeHTML(actor.name)}</strong> will apply <strong>${escapeHTML(choiceLabel)}</strong>.</p>
      ${branchRows ? `<section><h3>Selected Options</h3><ul>${branchRows}</ul></section>` : ""}
      <section>
        <h3>Actor Changes</h3>
        ${changeRows ? `<ul>${changeRows}</ul>` : "<p>No direct actor field changes.</p>"}
      </section>
      <section>
        <h3>Owned Items Created</h3>
        ${itemRows ? `<ul>${itemRows}</ul>` : "<p>No new owned Items.</p>"}
      </section>
    </div>
  `;
}

function choiceUpdateRows(actor, updates) {
  return Object.entries(updates)
    .map(([path, next]) => {
      const current = foundry.utils.getProperty(actor, path);
      if (String(current ?? "") === String(next ?? "")) return "";
      return `<li><strong>${escapeHTML(choicePathLabel(path))}</strong>: ${escapeHTML(displayValue(current))} -> ${escapeHTML(displayValue(next))}</li>`;
    })
    .filter(Boolean)
    .join("");
}

function choicePathLabel(path) {
  if (path.startsWith("system.skills.")) return `Skill ${skillLabel(path.split(".").pop())}`;
  if (path.startsWith("system.manifestations.")) return `Manifestation ${manifestationLabel(path.split(".").pop())}`;
  if (path.startsWith("system.resources.")) {
    const parts = path.split(".");
    const key = parts[2];
    return `Resource ${resourceLabel(key)}${parts[3] ? ` ${labelCase(parts[3])}` : ""}`;
  }
  if (path.startsWith("system.identity.")) return `Identity ${labelCase(path.split(".").pop())}`;
  return labelCase(path.replace(/^system\./, ""));
}
...
function skillLabel(skill) {
  return CONFIG.PTG.skills?.[skill] ?? labelCase(skill);
}

function manifestationLabel(manifestation) {
  return CONFIG.PTG.manifestations?.[manifestation] ?? labelCase(manifestation);
}
...
function embeddedAttachmentItems(attachments, sourceItem) {
  ...
  for (const attachment of normalizeAttachmentGrants(attachments)) {
    ...
    items.push({
      ...
      flags: canonicalEmbeddedFlags(sourceItem, "choice-attachment", {
        choiceKind: attachment.choiceKind ?? attachment.kind,
        choiceLabel
      })
    });
...
function simpleEmbeddedItem(type, grant, sourceItem) {
  ...
  return {
    ...
    system,
    flags: canonicalEmbeddedFlags(sourceItem, `choice-${type}`, {
      choiceLabel: name
    })
  };
}

function canonicalFlagUpdates(sourceItem, role, extra = {}) {
  const flags = canonicalEmbeddedFlags(sourceItem, role, extra)[SYSTEM_ID];

  return Object.fromEntries(Object.entries(flags).map(([key, value]) => [`flags.${SYSTEM_ID}.${key}`, value]));
}

function canonicalEmbeddedFlags(sourceItem, role, extra = {}) {
  return {
    [SYSTEM_ID]: {
      canonicalEmbeddedItem: true,
      canonicalId: canonicalItemId(sourceItem, role, extra),
      canonicalSource: "apply-choice",
      canonicalSourceUuid: sourceItem.uuid ?? "",
      canonicalSourceType: sourceItem.type ?? "",
      canonicalSourceName: sourceItem.name ?? "",
      canonicalRole: role,
      ...extra
    }
  };
...
function canonicalItemId(sourceItem, role, extra = {}) {
  const sourceKey = sourceItem.getFlag?.(SYSTEM_ID, "sourceId")
    ?? sourceItem.getFlag?.(SYSTEM_ID, "importId")
    ?? sourceItem.getFlag?.(SYSTEM_ID, "slug")
    ?? sourceItem.flags?.[SYSTEM_ID]?.sourceId
    ?? sourceItem.flags?.[SYSTEM_ID]?.importId
    ?? sourceItem.flags?.[SYSTEM_ID]?.slug
    ?? `${sourceItem.type ?? "item"}:${sourceItem.name ?? "item"}`;
  const suffix = extra.choiceLabel ?? extra.appliedChoiceKey ?? "";
  return [role, sourceItem.type ?? "item", slugify(sourceKey), slugify(suffix)].filter(Boolean).join(":");
}
