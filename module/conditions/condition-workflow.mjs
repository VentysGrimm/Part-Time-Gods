  ...
  return {
    ...
    flags: {
      [SYSTEM_ID]: {
        ...
        canonicalId: condition.canonicalId ?? `condition:custom:${slugify(name)}`,
        canonicalSource: condition.canonicalSource ?? "custom-condition",
        canonicalSourceUuid: condition.canonicalSourceUuid ?? "",
        canonicalSourceType: "condition",
        canonicalSourceName: name,
        canonicalRole: "condition"
      }
    }
  };
}
...
export async function conditionItemFromSelection(selection = {}) {
  ...
  source.flags = {
    ...
    [SYSTEM_ID]: {
      ...
      canonicalEmbeddedItem: true,
      canonicalId: premadeItem
        ? `condition:premade:${slugify(premadeItem.getFlag?.(SYSTEM_ID, "sourceId") ?? premadeItem.getFlag?.(SYSTEM_ID, "slug") ?? premadeItem.name)}`
        : source.flags?.[SYSTEM_ID]?.canonicalId ?? `condition:custom:${slugify(source.name)}`,
      canonicalSource: premadeItem ? "condition-selection-premade" : "condition-selection-custom",
      canonicalSourceUuid: premadeItem?.uuid ?? "",
      canonicalSourceType: "condition",
      canonicalSourceName: premadeItem?.name ?? source.name ?? "",
      canonicalRole: "condition"
    }
  };
...
async function postConditionChatCard(actor, condition, { result, sourceActor, sourceItem, reason } = {}) {
  ...
  await ChatMessage.create({
    ...
    content: `
      <div class="ptg-chat-card" data-ptg-chat-card="condition" data-actor-uuid="${escapeHTML(actor.uuid)}" data-item-uuid="${escapeHTML(condition.uuid ?? "")}" data-reason="${escapeHTML(reason ?? "Condition")}">
        ...
        <div class="ptg-chat-actions">
          ...
          <button type="button" data-ptg-chat-action="open-item">Open Condition</button>
          <button type="button" data-ptg-chat-action="condition-reduce">Reduce</button>
          <button type="button" data-ptg-chat-action="condition-recover">Recover</button>
        </div>
      </div>
    `
  });
