...
| Medium | Pantheon Pool as a first-class shared resource | 187-189 | Pantheon Actor has a pool field and character rolls can reference Pantheon Dice, but there is not yet a single table-facing pool spend/add workflow shared across characters. | Add a Pantheon Pool spend/add dialog that can post chat cards and optionally link characters to a Pantheon Actor. |
