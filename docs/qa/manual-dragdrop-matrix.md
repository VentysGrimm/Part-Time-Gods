...
| actor-territory-character | GM Territory interface | Actors sidebar Character actor | GM Territory grid or points panel | Character territory-ready attachments import or cleanly report no new territory-ready attachments. | Pending | Computer Use attempted physical drags from `Character` / `Character (2)` rows and portrait areas into the GM Territory grid and points panel on 2026-07-15; the app stayed at `3 points` / `2 influenced cells`, no warning appeared, and no native Foundry Actor payload was delivered under automation. On 2026-07-16, the Electron GM client opened the Actors sidebar beside the auto-opened Territory Grid app, confirmed imported `QA Character` territory data at `4 points` / `2 influenced cells`, and dragged the visible `QA Character` row text and portrait/icon into the Territory grid and Points panel. The app stayed at `4 points` / `2 influenced cells` with no warning. Source review shows the drop handler would warn on a delivered empty/non-character payload, so this attempt again indicates Computer Use did not deliver a native Foundry drop event. |
