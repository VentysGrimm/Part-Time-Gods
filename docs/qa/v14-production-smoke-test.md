...
| Automated tests | Pass | All 7 test modules pass through isolated `node --input-type=module --eval "await import(...)"` executions. Direct `node --test` cannot be run in the sandbox because Node path resolution hits `EPERM` on `C:\Users\Owner\AppData\Local\FoundryVTT\Data`. |
...
The 2026-07-08 desktop continuation found no new product defect: chat-command aliases are not part of the desired workflow, and `PTG GM Setup -> Combat Controls` opens the supported dialog. Issue #131 cannot close until the unresolved coverage gaps above are completed or explicitly deferred.
